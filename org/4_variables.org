...
#+DATE: <2022-05-26 Thu>
#+PROPERTY: header-args:C :main yes :includes <stdio.h> :exports both :results output :comments both :tangle yes
...
- To *code along during the lecture* using Emacs and Org-mode,
  download it from here and open it in Emacs with ~emacs --file~:
  [[https://tinyurl.com/4pe74kf9][tinyurl.com/4pe74kf9]].

- There is a separate Org-mode file available for *practice after the
  lecture*. Download it from here and open it in Emacs with ~emacs
  --file~: [[https://tinyurl.com/y44epjdc][tinyurl.com/y44epjdc]].
...
- [ ] We write a program to compute the dimensional ~weight~ of a box of
  ...
  12'' x 10'' x 8 ''. What does need to compile?
  #+begin_example C
...
- This is not what we need. When dividing one integer by another, C
  ...
  adding 165 to the volume before dividing by 166[fn:8] as shown in
  [[dweight]].
