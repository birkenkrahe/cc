...
- Open the Emacs browser with ~M-x eww~ on ~tinyurl.com/2p95hxrp~
...
- [ ] What is the output of [[switch]] for 5,3,0,-1,0.5?
  #+begin_quote
  | VALUE | OUTPUT        |
  |-------+---------------|
  |     5 | Failing       |
...
- [ ] Which problem/solution set does the program implement?
  #+begin_quote
